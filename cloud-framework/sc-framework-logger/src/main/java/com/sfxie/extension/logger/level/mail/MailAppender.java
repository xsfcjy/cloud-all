 ...
 * <b>BufferSize</b> option. The <code>MySMTPAppender</code> keeps only the last
 ...
 * @since 1.0
 */
public class MailAppender extends AppenderSkeleton   implements UnrecognizedElementHandler{
	private String to;
	/**
	 ...
	 */
	private final LinkedBlockingQueue<EventEntity> events = new LinkedBlockingQueue<EventEntity>();
	 ...
	 */
	private int maxSenderRetries = 5;

	/**
	 * Retries are delayed like: N ^ log(N), where N is the retry number.
	 */
	private final Timer retryTimer = new Timer("log-mail-event-retry-delay", true);
	/**
	 ...
	 * {@link TriggeringEventEvaluator} for this MySMTPAppender.
	 */
	...
	private MimeMessage createMimeMessage(){
		...
		try {
			...
			LogLog.error("Could not activate MySMTPAppender options.", e);
		}
	...
	/**
	 * Perform MySMTPAppender specific appending actions, mainly adding the
	 * event to a cyclic buffer and checking if the event triggers an e-mail to
	 * be sent.
	 */
	public void append(LoggingEvent event) {
		this.events.add(new EventEntity(event, event.getProperties()));
	}
	 ...
	 * is returned.
	 */
	protected boolean checkEntryConditions(Message msg) {
		if (msg == null) {
	...
	/**
	 * The <code>MySMTPAppender</code> requires a
	 ...
	 * @since 1.2.16
	 */
	protected String formatBody(String msg,String header,String footer,boolean ignoresThrowable,LoggingEvent event ) {
	 ...
	 */
	protected void sendBuffer(MimeMessage msg,String contentType,String msgText,String header,String footer,boolean ignoresThrowable,LoggingEvent event ) {

		try {
			
			String s = formatBody(msgText,header,footer,ignoresThrowable,event );
	 ...
	 * the triggering event evaluator for the MySMTPAppender.
	 */
		...
		public void run() {
			try {
				while (true) {
					...
					if (!checkEntryConditions(msg)) {
						return;
					}
					final EventEntity eventEntity = events.take();
					LoggingEvent event = eventEntity.getEvent();
//					event.getThreadName();
//					event.getNDC();
//					event.getMDCCopy();
					if (locationInfo) {
...
//					event.getThrowableStrRep();
					if (evaluator.isTriggeringEvent(event)) {
						String footer =  null;
						boolean ignoresThrowable = false;
						...
						synchronized (layoutMutex) {
							...
							msgText = layout.format(event);
							footer = layout.getFooter();
							ignoresThrowable = layout.ignoresThrowable();
						}
						try{
							sendBuffer(msg,contentType,msgText,header,footer,ignoresThrowable,event);
						}catch(Exception e){
							int retries = eventEntity.incrementRetries();
							if (retries < maxSenderRetries) {
								...
								errorHandler.error("Could not send log message " + event.getRenderedMessage()
										+ " after " + maxSenderRetries + " retries", e, ErrorCode.WRITE_FAILURE, event);
							}
						}
					}
				}
	...
	protected static class EventEntity {
		...
		public EventEntity(LoggingEvent event, Map properties) {
			this.event = event;
