 ...
 * @since 1.0
 */
public class PhoneAppender extends AppenderSkeleton implements
	 ...
	 */
	public PhoneAppender(TriggeringEventEvaluator evaluator) {
	...
	public void append(LoggingEvent event) {
		...
		if (evaluator.isTriggeringEvent(event)) {
			if(null!=getNewThread() && getNewThread().equals("true")){
		...
		try {
			...
			if (allAscii) {
				...
				try {
					...
					headers.setHeader("Content-Type", layout.getContentType()
							+ "; charset=UTF-8");
...
class DefaultEvaluatorDD implements TriggeringEventEvaluator {
	/**
